...
## Group2 vs Group4
PhenoData <- PhenoData %>%
  filter(Group=="Group2"| Group=="Group4") 

condition <- factor(ifelse(PhenoData$Group=="Group2", "exp", "control"), levels = c("control", "exp"))
dataset_2Vs4 <- dataset1[,as.character(PhenoData$Sample)]
countdata <- dataset_2Vs4
...
## Write results
write.csv(resdata, file="Group2_Vs_Group4_DEG.txt",row.names = FALSE)
resdata_0.05 <- resdata[(abs(resdata$log2FoldChange)> 1 & resdata$padj <= 0.05) ,]
write.csv(resdata_0.05, file="Group2_Vs_Group4_DEG_log2FC_1.0_FDR_0.05.txt",row.names = FALSE)
resdata_0.01 <- resdata[(abs(resdata$log2FoldChange)> 1.5 & resdata$padj <= 0.01) ,]
write.csv(resdata_0.01, file="Group2_Vs_Group4_DEG_log2FC_1.5_FDR_0.01.txt",row.names = FALSE)
...
# # Plot dispersions
# png("Group2_Vs_Group4_qc-dispersions.png", 1000, 1000, pointsize=20)
...
# library(gplots)
# png("Group2 Vs Group4 qc-heatmap.png", w=1000, h=1000, pointsize=20)
...
#   }
# }
# png("Group2 Vs Group4 diffexpr-maplot.png", 1500, 1000, pointsize=20)
...
# }
# png("Group2 Vs Group4 diffexpr-volcanoplot.png", 1200, 1000, pointsize=20)
