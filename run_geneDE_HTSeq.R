...
source("/research_jude/rgs01_jude/groups/blancgrp/projects/rRNA_variation/common/EMT_analysis_Nitish/differential_expression_analysis.TCC.R")

verb <- function(...) cat(sprintf(...), sep='', file=stdout())

fs <- list.files(path=".",pattern="htseq-annot", recursive=TRUE, full.names=TRUE)
...
simple_load_raw_counts_and_make_matrix_project  <-  function(fs){
	...
	colnames(indf) <- c("gene_id","expected_count")
...
simple_limma_voom_for_mrna('data2/database/Jake_Feb2021.txt',mtx_count=cmat, outfbase = 'blancgrp_211613_RNAseq_total_stranded_HTSeq', th_log2fc = log2(1.2), target_species = 'mm39')
### Change by Nitish
## Change RData file name
save.image("EMT_RNAseq_HTSeq.RData")
